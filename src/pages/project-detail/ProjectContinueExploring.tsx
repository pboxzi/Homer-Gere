...
import { ArrowRight, Film, ImageIcon, BookOpen } from 'lucide-react';
import { SECTION_IMAGES } from '../../data/images';

interface ProjectContinueExploringProps {
  onNavigate: (path: string) => void;
}

export const ProjectContinueExploring: React.FC<ProjectContinueExploringProps> = ({ onNavigate }) => {
  ...
  const CARDS = [
    {
      ...
      description: 'Explore Homer\'s complete filmography and upcoming productions.',
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {CARDS.map((card, idx) => (
            <motion.button
              key={card.id}
              onClick={card.action}
              className="group relative rounded-[1.5rem] overflow-hidden text-left bg-[#FAF9F7] hover:shadow-2xl hover:shadow-[#C9A84C]/8 transition-all duration-500 hover:-translate-y-1 focus:outline-none cursor-pointer"
