...
import { Camera, Save, Clock, User, Mail, Phone, MapPin, Globe, Calendar, Check, Shield } from 'lucide-react';
...
export const DashboardProfile: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Profile Completion */}
      <motion.div className="rounded-2xl border border-[#A6852F]/20 bg-gradient-to-r from-[#A6852F]/5 to-transparent p-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }}>
      ...
      <motion.div className="space-y-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
        <div className="flex items-center gap-5">
          <div className="relative">
            <div className="w-20 h-20 rounded-2xl bg-[#A6852F]/10 flex items-center justify-center text-[#A6852F] text-2xl font-editorial overflow-hidden shadow-sm">
      ...
      <motion.div className="space-y-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
        ...
        <div>
          ...
          <textarea value={formData.biography} onChange={(e) => update('biography', e.target.value)} rows={4} className="w-full px-4 py-3 rounded-xl bg-white border border-[#A6852F]/20 text-sm text-[#1C1917] focus:outline-none focus:ring-2 focus:ring-[#A6852F]/30 resize-none shadow-sm" placeholder="Tell us about yourself..." />
        </div>
...
const Field: React.FC<{ label: string; value: string; type?: string; onChange: (v: string) => void; icon?: React.ReactNode }> = ({ label, value, type = 'text', onChange, icon }) => (
  <div>
    ...
    <div className="relative">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className={`w-full py-3 rounded-xl bg-white border border-[#A6852F]/20 text-sm text-[#1C1917] focus:outline-none focus:ring-2 focus:ring-[#A6852F]/30 shadow-sm ${icon ? 'pl-10 pr-4' : 'px-4'}`} />
    </div>
  </div>
);
...
const SelectField: React.FC<{ label: string; value: string; options: string[]; onChange: (v: string) => void; icon?: React.ReactNode }> = ({ label, value, options, onChange, icon }) => (
  <div>
    ...
    <div className="relative">
      {icon && <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[#57534E]/40">{icon}</div>}
      <select value={value} onChange={(e) => onChange(e.target.value)} className={`w-full py-3 rounded-xl bg-white border border-[#A6852F]/20 text-sm text-[#1C1917] focus:outline-none focus:ring-2 focus:ring-[#A6852F]/30 shadow-sm appearance-none ${icon ? 'pl-10 pr-4' : 'px-4'}`}>
