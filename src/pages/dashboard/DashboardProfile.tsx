...
export const DashboardProfile: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Profile Completion */}
      <motion.div className="rounded-2xl border border-[#A6852F]/35 bg-gradient-to-r from-[#A6852F]/5 to-transparent p-4 shadow-lg shadow-[#A6852F]/15" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }}>
      ...
      <motion.div className="space-y-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
        <div className="flex items-center gap-5">
          <div className="relative">
            <div className="w-20 h-20 rounded-2xl bg-[#A6852F]/10 flex items-center justify-center text-[#A6852F] text-2xl font-editorial overflow-hidden shadow-lg shadow-[#A6852F]/15">
            ...
            <label htmlFor="avatar-upload" className="absolute -bottom-1 -right-1 w-7 h-7 rounded-lg bg-[#1C1917] text-white flex items-center justify-center hover:bg-[#292524] transition-colors cursor-pointer shadow-md shadow-[#A6852F]/10">
      ...
      <motion.div className="space-y-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
        ...
        <div>
          ...
          <textarea value={formData.biography} onChange={(e) => update('biography', e.target.value)} rows={4} className="w-full px-4 py-3 rounded-xl bg-white border border-[#A6852F]/30 text-sm text-[#1C1917] focus:outline-none focus:ring-2 focus:ring-[#A6852F]/30 resize-none shadow-md shadow-[#A6852F]/8" placeholder="Tell us about yourself..." />
        </div>

        {/* Save */}
        <div className="flex items-center gap-3 pt-2">
          <button onClick={handleSave} className="inline-flex items-center gap-2 bg-[#A6852F] hover:bg-[#8B6F1F] active:scale-95 text-white font-medium text-sm px-6 py-2.5 rounded-2xl transition-all duration-300 cursor-pointer shadow-lg shadow-[#A6852F]/25">
...
const Field: React.FC<{ label: string; value: string; type?: string; onChange: (v: string) => void; icon?: React.ReactNode }> = ({ label, value, type = 'text', onChange, icon }) => (
  <div>
    ...
    <div className="relative">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className={`w-full py-3 rounded-xl bg-white border border-[#A6852F]/25 text-sm text-[#1C1917] focus:outline-none focus:ring-2 focus:ring-[#A6852F]/30 shadow-sm shadow-[#A6852F]/5 ${icon ? 'pl-10 pr-4' : 'px-4'}`} />
    </div>
  </div>
);
...
const SelectField: React.FC<{ label: string; value: string; options: string[]; onChange: (v: string) => void; icon?: React.ReactNode }> = ({ label, value, options, onChange, icon }) => (
  <div>
    ...
    <div className="relative">
      {icon && <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[#57534E]/40">{icon}</div>}
      <select value={value} onChange={(e) => onChange(e.target.value)} className={`w-full py-3 rounded-xl bg-white border border-[#A6852F]/25 text-sm text-[#1C1917] focus:outline-none focus:ring-2 focus:ring-[#A6852F]/30 shadow-sm shadow-[#A6852F]/5 appearance-none ${icon ? 'pl-10 pr-4' : 'px-4'}`}>
