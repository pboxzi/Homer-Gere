    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <motion.div key={s.label} className="rounded-xl p-3 text-center border border-[#A6852F]/10 bg-white shadow-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 + i * 0.05 }}>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search downloads..." className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white border border-[#A6852F]/20 text-sm focus:outline-none focus:ring-2 focus:ring-[#A6852F]/30" />
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map((item, i) => {
            ...
            return (
              <motion.div key={item.id} className="flex items-center gap-4 p-4 rounded-2xl border border-[#A6852F]/20 bg-white hover:border-[#A6852F]/35 transition-all duration-300 shadow-sm hover:shadow-md" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 + i * 0.04 }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${catConfig.color}12`, color: catConfig.color }}>
                ...
                <button onClick={() => handleDownload(item)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium transition-all cursor-pointer ${isDownloaded ? 'bg-[#16A34A]/10 text-[#16A34A] hover:bg-[#16A34A]/20' : 'bg-[#A6852F] text-white hover:bg-[#8B6F24]'}`}>
