...
import { FileText, Clock, CheckCircle, XCircle, AlertCircle, Hourglass } from 'lucide-react';
import { MOCK_REQUESTS, RequestStatus } from '../../data/dashboardData';
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {Object.entries(STATUS_CONFIG).map(([key, config], i) => {
          const count = MOCK_REQUESTS.filter((r) => r.status === key).length;
          return (
            <motion.div
              key={key}
      ...
      <div className="space-y-3">
        {MOCK_REQUESTS.map((r, i) => {
          ...
          return (
            <motion.div
              key={r.id}
              className="flex items-start gap-4 p-5 rounded-2xl border border-[#E8E5DF]/60 bg-white hover:border-[#A6852F]/20 transition-all cursor-pointer"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: 0.2 + i * 0.05 }}
            >
              <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: status.bg }}>
