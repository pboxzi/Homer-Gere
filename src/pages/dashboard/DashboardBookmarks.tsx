...
export const DashboardBookmarks: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {bookmarks.length === 0 ? (
        <motion.div
          className="rounded-2xl border border-dashed border-[#E8E5DF] bg-[#F3F1ED]/30 p-12 text-center"
        ...
        <div className="space-y-3">
          {bookmarks.map((b, i) => (
            <motion.div
              key={b.id}
              className="flex items-center gap-4 p-4 rounded-2xl border border-[#E8E5DF]/60 bg-white hover:border-[#A6852F]/20 transition-all"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
