...
import { MOCK_CONVERSATIONS, MOCK_MEMBERSHIP } from '../../data/dashboardData';

export const DashboardChat: React.FC = () => {
  const navigate = useNavigate();
  const canOpenWhatsApp = MOCK_MEMBERSHIP.plan === 'Gold' || MOCK_MEMBERSHIP.plan === 'Platinum';
    ...
    <div className="space-y-8">
      ...
      >
        <button
          ...
          className="w-full flex items-center gap-4 p-5 rounded-2xl border border-dashed border-[#A6852F]/30 hover:border-[#A6852F]/60 hover:bg-[#A6852F]/5 transition-all duration-300 cursor-pointer group"
        >
      ...
      {canOpenWhatsApp && (
        ...
        >
          <button
            onClick={() => {}}
            className="w-full flex items-center gap-4 p-5 rounded-2xl border border-[#25D366]/20 hover:border-[#25D366]/40 hover:bg-[#25D366]/5 transition-all duration-300 cursor-pointer group"
          >
            <div className="w-12 h-12 rounded-2xl bg-[#25D366]/10 flex items-center justify-center text-[#25D366] group-hover:bg-[#25D366] group-hover:text-white transition-all duration-500">
              <Phone className="w-5 h-5" />
            </div>
            <div className="flex-1 text-left">
              <p className="text-sm font-medium text-[#1C1917] group-hover:text-[#25D366] transition-colors">Open Official WhatsApp</p>
              <p className="text-xs text-[#57534E]">Available for {MOCK_MEMBERSHIP.plan} members</p>
            </div>
