      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-6">
          {MEMBERSHIP_STEPS.map((step, idx) => (
            <motion.div
              key={step.id}
              className="flex items-start gap-6 p-6 sm:p-8 rounded-[1.5rem] bg-white border border-[#E8E5DF]/60 hover:border-[#C9A84C]/20 transition-colors duration-300"
              initial={{ opacity: 0, y: 25 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.6, delay: 0.1 + idx * 0.08, ease: [0.22, 1, 0.36, 1] }}
            >
              {/* Step Number */}
              <div className="shrink-0 w-12 h-12 rounded-2xl bg-[#C9A84C]/10 flex items-center justify-center text-[#C9A84C]">
                {getStepIcon(step.icon)}
              </div>

              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-[10px] font-medium text-[#C9A84C] bg-[#C9A84C]/10 px-2.5 py-0.5 rounded-full">
                    Step {step.id}
                  </span>
