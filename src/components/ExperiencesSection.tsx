...
export const ExperiencesSection: React.FC<ExperiencesSectionProps> = ({
  ...
  const getExperienceIcon = (iconName: string) => {
    switch (iconName) {
      case 'users': return <Users className="w-5 h-5 text-[#C8A96A]" />;
      case 'video': return <Video className="w-5 h-5 text-[#C8A96A]" />;
      case 'user-check': return <UserCheck className="w-5 h-5 text-[#C8A96A]" />;
      case 'pen-tool': return <PenTool className="w-5 h-5 text-[#C8A96A]" />;
      case 'star': return <Star className="w-5 h-5 text-[#C8A96A]" />;
      case 'grid': return <Grid className="w-5 h-5 text-[#C8A96A]" />;
      default: return <Star className="w-5 h-5 text-[#C8A96A]" />;
    }
  };
          ...
          {EXPERIENCES.map((exp) => (
            ...
            >
              <div>
                ...
                <h3 className="text-sm font-editorial font-bold text-[#111827] group-hover:text-[#C8A96A] transition-colors duration-300">
                  {exp.title}
                </h3>
              ...
              <div className="mt-5 pt-4 border-t border-[#E4DFD5] flex items-center justify-between">
                ...
                <ArrowRight className="w-3.5 h-3.5 text-[#D1D5DB] group-hover:text-[#C8A96A] group-hover:translate-x-0.5 transition-all duration-300" />
              </div>
            </div>
          ))}
        </div>
