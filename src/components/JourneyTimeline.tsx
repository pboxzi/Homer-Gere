        ...
        <div className="relative py-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-4 relative z-10">
            {timelineMilestones.map((item) => {
              ...
              return (
                <button key={item.id} onClick={() => { setActiveId(item.id); onSelectMilestone(item); }} className={`flex flex-col items-center text-center group p-4 transition-all duration-300 focus:outline-none cursor-pointer rounded-2xl ${isSelected ? 'bg-[#A6852F]/8' : 'hover:bg-[#F3F1ED]/60'}`}>
                  <div className={`w-14 h-14 rounded-full flex items-center justify-center transition-all duration-500 ${isToday || isSelected ? 'bg-[#A6852F] text-white shadow-lg shadow-[#A6852F]/25' : 'bg-[#F3F1ED] text-[#57534E] group-hover:bg-[#A6852F]/15 group-hover:text-[#A6852F]'}`}>
