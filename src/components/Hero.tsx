...
export const Hero: React.FC<HeroProps> = ({ onExploreJourney, onViewProject, onOpenChat }) => {
  const getMetricIcon = (iconName: string) => {
    switch (iconName) {
      case 'clapperboard': return <Clapperboard className="w-4 h-4 text-[#C9A84C]" />;
      case 'star': return <Star className="w-4 h-4 text-[#C9A84C]" />;
      case 'globe': return <Globe className="w-4 h-4 text-[#C9A84C]" />;
      case 'users': return <Users className="w-4 h-4 text-[#C9A84C]" />;
      default: return <Star className="w-4 h-4 text-[#C9A84C]" />;
    }
  };
        ...
        <div id="hero-container" className="relative rounded-[2rem] overflow-hidden bg-[#F3F1ED] min-h-[680px] lg:min-h-[720px] w-full flex flex-col justify-between group transition-all duration-300">
          ...
          <div className="relative z-10 p-6 sm:p-10 md:p-14 lg:p-16 lg:pb-12 w-full lg:w-[48%] flex flex-col items-start justify-between h-full space-y-6 lg:space-y-0">
            <div className="flex flex-col items-start gap-5 sm:gap-6 max-w-xl">
              <div className="space-y-3">
                <p className="text-[11px] font-medium tracking-[0.2em] text-[#C9A84C] uppercase">Official Website</p>
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 mt-1">
                <button onClick={() => onOpenChat?.('fan')} className="inline-flex items-center justify-center gap-2.5 bg-[#C9A84C] hover:bg-[#B8983A] active:scale-95 text-white font-medium text-sm px-7 py-3.5 rounded-2xl transition-all duration-300 hover:shadow-lg hover:shadow-[#C9A84C]/25 focus:outline-none cursor-pointer">
                ...
                <button onClick={() => onViewProject?.(FEATURED_PROJECT.id)} className="inline-flex items-center justify-center gap-2 bg-transparent hover:bg-[#F3F1ED] active:scale-95 text-[#1C1917] font-medium text-sm px-6 py-3.5 rounded-2xl transition-all duration-300 focus:outline-none cursor-pointer group/btn">
                  <Play className="w-3.5 h-3.5 text-[#C9A84C] fill-[#C9A84C]" /><span>Latest Project</span>
                  <ArrowRight className="w-3.5 h-3.5 text-[#57534E] group-hover/btn:text-[#C9A84C] group-hover/btn:translate-x-0.5 transition-all duration-300" />
                </button>
              </div>
            </div>
            {/* Statistics */}
                ...
                {METRICS.map((metric, idx) => (
                  <div key={idx} className="flex flex-col items-start p-4 transition-all duration-300 rounded-2xl hover:bg-[#F3F1ED]/60">
                    <div className="flex items-center justify-between w-full mb-2">
                      <div className="w-8 h-8 rounded-xl bg-[#C9A84C]/10 flex items-center justify-center shrink-0">{getMetricIcon(metric.icon)}</div>
          ...
          <div className="relative z-10 pt-1 pb-4 w-full flex flex-col items-center justify-center">
            <a href="#projects" className="group flex flex-col items-center gap-2 focus:outline-none cursor-pointer" aria-label="Scroll to explore content">
              <span className="text-[10px] font-medium text-[#A8A29E] group-hover:text-[#C9A84C] transition-colors uppercase tracking-[0.25em]">Scroll to Explore</span>
