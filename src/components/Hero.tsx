          ...
          <div className="relative z-10 p-4 sm:p-10 md:p-14 lg:p-16 lg:pb-12 w-full lg:w-[48%] flex flex-col items-start justify-between h-full space-y-4 lg:space-y-0">
            <div className="flex flex-col items-start gap-3 sm:gap-6 max-w-xl">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-0 sm:mt-1">
                {heroButtonLink ? (
                  <a href={heroButtonLink} className="inline-flex items-center justify-center gap-2 bg-[#A6852F] hover:bg-[#B8983A] active:scale-95 text-white font-medium text-xs sm:text-sm px-5 sm:px-7 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl transition-all duration-300 hover:shadow-lg hover:shadow-[#A6852F]/25 focus:outline-none">
                    <MessageSquare className="w-3.5 h-3.5 sm:w-4 sm:h-4" /><span>{heroButtonText}</span>
                  </a>
                ) : (
                  <button onClick={onOpenChat} className="inline-flex items-center justify-center gap-2 bg-[#A6852F] hover:bg-[#B8983A] active:scale-95 text-white font-medium text-xs sm:text-sm px-5 sm:px-7 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl transition-all duration-300 hover:shadow-lg hover:shadow-[#A6852F]/25 focus:outline-none cursor-pointer">
                    <MessageSquare className="w-3.5 h-3.5 sm:w-4 sm:h-4" /><span>{heroButtonText}</span>
                  </button>
                )}
                {secondaryButtonLink ? (
                  <a href={secondaryButtonLink} className="inline-flex items-center justify-center gap-1.5 sm:gap-2 bg-transparent hover:bg-[#F3F1ED] active:scale-95 text-[#1C1917] font-medium text-xs sm:text-sm px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl transition-all duration-300 focus:outline-none group/btn">
                    <Play className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#A6852F] fill-[#A6852F]" /><span>{secondaryButtonText}</span>
                  ...
                  <button onClick={() => onViewProject?.(featuredProject.id)} className="inline-flex items-center justify-center gap-1.5 sm:gap-2 bg-transparent hover:bg-[#F3F1ED] active:scale-95 text-[#1C1917] font-medium text-xs sm:text-sm px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl transition-all duration-300 focus:outline-none cursor-pointer group/btn">
                    <Play className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#A6852F] fill-[#A6852F]" /><span>{secondaryButtonText}</span>
                    <ArrowRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#57534E] group-hover/btn:text-[#A6852F] group-hover/btn:translate-x-0.5 transition-all duration-300" />
                  </button>
                )}
