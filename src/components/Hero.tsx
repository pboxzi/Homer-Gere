...
export const Hero: React.FC<HeroProps> = ({ onExploreJourney, onViewProject, onOpenChat }) => {
  ...
  return (
    <section id="home" className="pt-24 pb-0 bg-[#F5F2EB] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Hero Container */}
        <div
          id="hero-container"
          className="relative rounded-[2rem] overflow-hidden bg-[#EDE9E0] min-h-[600px] lg:aspect-[16/9] w-full flex flex-col justify-between group transition-all duration-300"
        >
          ...
          <div className="relative lg:absolute top-0 right-0 w-full lg:w-[55%] h-[340px] sm:h-[420px] lg:h-full shrink-0">
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-[#F5F2EB] via-[#F5F2EB]/40 to-transparent hidden lg:block pointer-events-none" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#F5F2EB] via-[#F5F2EB]/50 to-transparent lg:hidden pointer-events-none" />
          </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {METRICS.map((metric, idx) => (
                  ...
                  >
                    ...
                    <div className="text-[11px] text-[#78716C] font-medium tracking-wide mt-0.5 uppercase">
                      {metric.label}
                    </div>
                  </div>
                ))}
