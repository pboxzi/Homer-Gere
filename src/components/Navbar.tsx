...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    ...
    >
      <div className={`max-w-7xl mx-auto px-4 sm:px-8 lg:px-10 flex items-center justify-between transition-all duration-500 ${scrolled ? 'h-12' : 'h-14 sm:h-16'}`}>
        {/* Brand */}
        <button
          onClick={() => handleNavClick('home')}
          className="group flex flex-col text-left focus:outline-none cursor-pointer"
        >
          <span className={`font-editorial tracking-[0.06em] text-[#1C1917] group-hover:text-[#A6852F] transition-all duration-500 uppercase leading-none ${scrolled ? 'text-sm' : 'text-base sm:text-xl'}`}>
            Homer Gere
          </span>
            <span className={`font-medium tracking-[0.35em] text-[#A6852F]/70 uppercase transition-all duration-500 ${scrolled ? 'text-[9px] mt-0.5' : 'text-[9px] sm:text-[11px] mt-0.5 sm:mt-1'}`}>
            Official Website
          </span>
        </button>
        ...
        <div className="hidden lg:flex items-center gap-3">
          {isAuthenticated ? (
            ...
            <button onClick={() => routerNavigate('/auth/sign-in')} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full border border-[#A6852F]/40 bg-[#A6852F]/10 text-[#A6852F] text-xs font-medium hover:bg-[#A6852F]/20 hover:border-[#A6852F]/60 transition-all cursor-pointer shadow-[0_0_12px_rgba(166,133,47,0.15)] hover:shadow-[0_0_20px_rgba(166,133,47,0.25)]">
